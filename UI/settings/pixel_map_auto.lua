...
function pixel_map_auto:load(loveframes, frame, tabs, start_y, step_y, parent)
	...
	tabs:AddTab("Auto", parent.panel_pixel_map_auto, nil, "ressource/icons/map.png")
	local step_y = 25
	...
	self.numberbox_x:SetWidth(parent.panel_pixel_map_auto:GetWidth()-16-100)
	...
	self.numberbox_y:SetWidth(parent.panel_pixel_map_auto:GetWidth()-16-100)
	...
	self.id:SetWidth(parent.panel_pixel_map_auto:GetWidth()-16-100)
...
function pixel_map_auto:click(x,y)
	...
	for px=1, self.parent.select_lx do
		for py=1, self.parent.select_ly do
			print(px,py)
			...
			self.parent.map[off_x+px][off_y+py] = {
				...
				net = self.net:GetValue(),
				subnet = self.subnet:GetValue(),
				uni = 0,--self.uni:GetValue(),
				id = (px-1)+(py-1)*5,--self.id:GetValue()
			}
		end
	end
	self.parent:reload()
